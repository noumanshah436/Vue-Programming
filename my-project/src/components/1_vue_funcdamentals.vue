<template>
  ...
  <form @submit.prevent="submitForm">
    ...
    <div>
      <input id="rememberMe" type="checkbox" v-model="formValues.rememberMe" />
...
export default {
  ...
  computed: {
    ...
    fullName: {
      get() {
        // called when we get fullName property
        return `${this.firstName} ${this.lastName}`;
      },
      set(value) {
        // called when we set fullName property
  ...
  watch: {
    ...
    volume: {
      handler(newValue, oldValue) {
        //  same function we write alone above
        if (newValue > oldValue && newValue === 16) {
          alert(
            "Listening to a high volumne for a long time may damage your hearing"
          );
